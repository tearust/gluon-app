...
export default class {
      ...
      if (event.section === 'gluon') {
        ...
        switch (event.method) {
          case 'RegistrationApplicationSucceed':
            console.log(
              'new pair found, app:',
              encodeAddress(data[0]),
              'browser:',
              encodeAddress(data[1]),
            );
            if (this.callback.RegistrationApplicationSucceed) {
              this.callback.RegistrationApplicationSucceed(
                encodeAddress(data[0]),
                encodeAddress(data[1]),
              );
            }
  ...
  async sendNonceForPairMobileDevice(nonce, account_address) {
    ...
    await this.promisify(async (cb) => {
      await this.api.tx.gluon
        .browserSendNonce(nonce_hex)
  ...
  async getSelectDelegator(startPosition = 0) {
    ...
    const delegates = await this.api.rpc.tea.getDelegates(
      startPosition,
      neededDelegatesCount,
    );
  ...
  async browserGenerateAccount(account, key, delegate_rsa) {
    ...
    const p1 = hexToU8a(
      '0x02ef26ecbd873ee5a71c7e16a34ef540c17a81cf7530e418ea9f6165092dc2541a',
    );
    const p2_n = 3;
    const p2_k = 2;
    ...
    return await this.promisify(async (cb) => {
      ...
      await this.api.tx.gluon
        .browserGenerateAccount(me_nonce_hash, encoded_hash)
  ...
  async getAssetsByAddress(address) {
    ...
    const multi_sig_accounts = await this.api.query.gluon.browserMultiSigAccounts(
      pub,
    );
  ...
  async browserSignTx(account, data, nonce_hash, nonce_rsa) {
    ...
    return this.promisify(async (cb) => {
      await this.api.tx.gluon
        .browserSignTx(data_adhoc, nonce_hash, nonce_rsa)
        .signAndSend(account, (param) => {
